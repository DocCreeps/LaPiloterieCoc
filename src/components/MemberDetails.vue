  ...
  methods: {
    ...
    isSuperTroop(troopName) {
      const superTroopNames = [
        'Super Barbarian', 'Super Archer', 'Super Giant', 'Super Wall Breaker', 'Sneaky Goblin', 'Super Witch', 'Super Minion', 'Super Valkyrie', 'Super Dragon', 'Super Bowler',
        'Super Wizard', 'Super Pekka', 'Super Miner', 'Super Hog Rider', 'Rocket Balloon', 'Inferno Dragon', 'Ice Hound'
      ];
      return superTroopNames.includes(troopName);
    },
    isSiege(troopName) {
      const siegeNames = [
        'Wall Wrecker', 'Battle Blimp', 'Stone Slammer', 'Siege Barracks', 'Troop Launcher', 'Battle Drill','Log Launcher', 'Flame Flinger' // Ajoutez les noms des machines de siège ici
      ];
      return siegeNames.includes(troopName);
    },
    isHeroPet(troopName) {
      const heroPetNames = [
        'L.A.S.S.I', 'Electro Owl', 'Mighty Yak', 'Unicorn', 'Phoenix', 'Poison Lizard', 'Diggy', 'Frosty', 'Spirit Fox', 'Angry Jelly'  // Ajoutez les noms des Familiers ici
      ];
...
computed: {
  ...
  regularTroops() {
    return this.member.troops.filter(troop => troop.village === 'home' && !this.isSuperTroop(troop.name) && !this.isHeroPet(troop.name) && !this.isSiege(troop.name));
  },
  superTroops() {
...
<!--<h3 class="text-xl font-semibold mt-4">Super Troupes</h3>-->
<!--<div class="mt-2">-->
<!--<div v-for="troop in superTroops" :key="troop.name" class="flex items-center mb-2">-->
<!--  <span>{{ troop.name }} (Niveau {{ troop.level }})</span>-->
<!--</div>-->
<!--</div>-->
